...
export default function ServiceProviderProfileForm({ userId: propUserId }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-5xl mx-auto bg-[#FFF3E0] p-8 rounded-3xl shadow-xl space-y-6">
      ...
      {/* Business Info */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <input required name="businessName" type="text" placeholder="Business Name" className="input" value={formData.businessName} onChange={e => setFormData({ ...formData, businessName: e.target.value })} />
        <input required name="businessLicenseNumber" type="text" placeholder="License Number" className="input" value={formData.businessLicenseNumber} onChange={e => setFormData({ ...formData, businessLicenseNumber: e.target.value })} />
        <input required name="gstNumber" type="text" placeholder="GST Number" className="input" value={formData.gstNumber} onChange={e => setFormData({ ...formData, gstNumber: e.target.value })} />
        <label className="flex items-center gap-2 text-sm">
          <input name="needOfDeliveryAgent" type="checkbox" className="accent-orange-600" checked={formData.needOfDeliveryAgent} onChange={e => setFormData({ ...formData, needOfDeliveryAgent: e.target.checked })} /> Need Delivery Agent?
        </label>
      </div>
        ...
        <ul className="mt-4 space-y-2">
          {formData.priceDTO.map(p => {
            ...
            return (
              <li key={p.item.itemId} className="bg-white p-3 rounded-md flex justify-between items-center shadow">
                <span>{fullItem?.itemName || p.item.itemId} — ₹{p.price}</span>
                <button type="button" onClick={() => removePriceItem(p.item.itemId)} className="text-red-500 hover:underline">Remove</button>
              </li>
            );
          })}
        </ul>
      ...
      {/* Bank Details */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <input name="accountHolderName" type="text" placeholder="Account Holder Name" className="input" value={formData.bankAccount.accountHolderName} onChange={e => setFormData({ ...formData, bankAccount: { ...formData.bankAccount, accountHolderName: e.target.value } })} />
        <input name="bankName" type="text" placeholder="Bank Name" className="input" value={formData.bankAccount.bankName} onChange={e => setFormData({ ...formData, bankAccount: { ...formData.bankAccount, bankName: e.target.value } })} />
        <input name="ifscCode" type="text" placeholder="IFSC Code" className="input" value={formData.bankAccount.ifscCode} onChange={e => setFormData({ ...formData, bankAccount: { ...formData.bankAccount, ifscCode: e.target.value } })} />
        <input name="bankAccountNumber" type="text" placeholder="Bank Account Number" className="input" value={formData.bankAccount.bankAccountNumber} onChange={e => setFormData({ ...formData, bankAccount: { ...formData.bankAccount, bankAccountNumber: e.target.value } })} />
      </div>
      ...
      <div>
        <h4 className="text-xl font-semibold mb-3">Upload Documents</h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <label className="flex flex-col">Aadhar Card (required)
            <input type="file" name="aadharCard" onChange={handleFileChange} className="file-input" required />
          </label>
          <label className="flex flex-col">PAN Card (optional)
            <input type="file" name="panCard" onChange={handleFileChange} className="file-input" />
          </label>
          <label className="flex flex-col">Utility Bill (required)
            <input type="file" name="utilityBill" onChange={handleFileChange} className="file-input" required />
          </label>
          <label className="flex flex-col">Profile Photo (required)
            <input type="file" name="profilePhoto" onChange={handleFileChange} className="file-input" required />
          </label>
        </div>
      </div>
