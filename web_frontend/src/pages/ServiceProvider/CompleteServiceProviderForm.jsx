...
// Author: Bhavika
// Description: Complete profile page for service provider with multipart file handling like CompleteDeliveryAgentProfilePage
...
export default function ServiceProviderProfileForm({ userId: propUserId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (files.utilityBill) formDataToSend.append("utilityBill", files.utilityBill);
      if (files.profilePhoto) formDataToSend.append("profilePhoto", files.profilePhoto);

      await axios.post(`${BASE_URL}/sp/complete-sp-profile/${userId}`, formDataToSend, {
        headers: { "Content-Type": "multipart/form-data" }
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-5xl mx-auto bg-[#FFF3E0] p-8 rounded-3xl shadow-xl space-y-6">
      ...
      {/* Business Info */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <input required name="businessName" type="text" placeholder="Business Name" className="input"
          onChange={e => setFormData({ ...formData, businessName: e.target.value })} />
        <input required name="businessLicenseNumber" type="text" placeholder="License Number" className="input"
          onChange={e => setFormData({ ...formData, businessLicenseNumber: e.target.value })} />
        <input required name="gstNumber" type="text" placeholder="GST Number" className="input"
          onChange={e => setFormData({ ...formData, gstNumber: e.target.value })} />
        <label className="flex items-center gap-2 text-sm">
          <input name="needOfDeliveryAgent" type="checkbox" className="accent-orange-600"
            onChange={e => setFormData({ ...formData, needOfDeliveryAgent: e.target.checked })} /> Need Delivery Agent?
        </label>
      </div>
      ...
      {/* Bank Details */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <input name="accountHolderName" type="text" placeholder="Account Holder Name" className="input"
          onChange={e => setFormData({ ...formData, bankAccount: { ...formData.bankAccount, accountHolderName: e.target.value } })} />
        <input name="bankName" type="text" placeholder="Bank Name" className="input"
          onChange={e => setFormData({ ...formData, bankAccount: { ...formData.bankAccount, bankName: e.target.value } })} />
        <input name="ifscCode" type="text" placeholder="IFSC Code" className="input"
          onChange={e => setFormData({ ...formData, bankAccount: { ...formData.bankAccount, ifscCode: e.target.value } })} />
        <input name="bankAccountNumber" type="text" placeholder="Bank Account Number" className="input"
          onChange={e => setFormData({ ...formData, bankAccount: { ...formData.bankAccount, bankAccountNumber: e.target.value } })} />
      </div>
