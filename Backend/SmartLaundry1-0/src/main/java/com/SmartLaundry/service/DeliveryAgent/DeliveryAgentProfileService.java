...
public class DeliveryAgentProfileService {
    ...
    public String completeProfile(String userId, DeliveryAgentCompleteProfileRequestDTO data,
        ...
        emailService.sendMail(user.getEmail(), "Delivery Agent Profile Submission",
                "Congratulations! Your request to become a Delivery Agent has been submitted for review.");

        return "Your request is sent successfully. Wait for a response.";
    }

    @Transactional
    public String editDetail(String userId, DeliveryAgentProfileDTO dto,
                             MultipartFile aadharCard, MultipartFile panCard,
                             MultipartFile drivingLicense, MultipartFile profilePhoto) throws IOException {
        ...
        if (dto.getDateOfBirth() != null) agent.setDateOfBirth(dto.getDateOfBirth());
        if (dto.getVehicleNumber() != null) agent.setVehicleNumber(dto.getVehicleNumber());
        if (dto.getBankName() != null) agent.setBankName(dto.getBankName());
        if (dto.getAccountHolderName() != null) agent.setAccountHolderName(dto.getAccountHolderName());
        if (dto.getIfscCode() != null) agent.setIfscCode(dto.getIfscCode());
        if (dto.getGender() != null) agent.setGender(dto.getGender());
        if (aadharCardPath != null) agent.setAadharCardPhoto(aadharCardPath);
        if (panCardPath != null) agent.setPanCardPhoto(panCardPath);
        if (drivingLicensePath != null) agent.setDrivingLicensePhoto(drivingLicensePath);
